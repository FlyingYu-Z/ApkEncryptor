		...
		if (mStringsCount > 0)
		{
			...
			for (int i =0; i < mStringsCount ; i++)
			{
				if ((mEncoder & 0x100) != 0)
				{
					...
					offset += varint[1];
					int length = varint[0];
					mStrings.add(i,new String(rawStrings,offset, length, Charset.forName("UTF-8")));
					//mStrings.add(Arrays.toString(mPerStrOffset));
				}
				else
				{
					int offset = mPerStrOffset[i];
			...
			//throw new IOException(Arrays.toString( mStrings.toArray()));
		}
...
//					size += writer.writeByteArray(getVarBytes(s.toCharArray().length));
//					size += writer.writeByteArray(getVarBytes(raw.length));
//					size += writer.writeByteArray(raw);
//					size += writer.writeByte(0);
//				}
//			}
//			else
//			{
